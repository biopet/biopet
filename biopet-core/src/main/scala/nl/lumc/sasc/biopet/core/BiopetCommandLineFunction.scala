...
trait BiopetCommandLineFunction extends CommandLineResources { biopetFunction =>
  ...
  override def freezeFieldValues() {
    ...
    if (jobOutputFile != null) {
      ...
      this match {
        case s: ScatterGatherableFunction =>
          s.scatterGatherDirectory = new File(this.commandDirectory, ".scatter")
        case _ =>
      }
    }
        ...
        if (!BiopetCommandLineFunction.executableCache.contains(executable)) {
          try {
            ...
            case ioe: java.io.IOException =>
              logger.warn(
                s"Could not use 'which' on '$executable', check on executable skipped: " + ioe)
              BiopetCommandLineFunction.executableCache += executable -> executable
          }
        }
