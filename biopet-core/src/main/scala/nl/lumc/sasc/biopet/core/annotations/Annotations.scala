...
trait AnnotationGtf extends BiopetQScript with Reference { qscript: QScript =>
  ...
  lazy val annotationGtf: File = {
    ...
    inputFiles :+ InputFile(
      file,
      config("annotation_gtf_md5", freeVar = true, extraSubPath = geneAnnotationSubPath))
    file
  }
}
trait AnnotationGff extends BiopetQScript with Reference { qscript: QScript =>
  ...
  lazy val annotationGff: File = {
    ...
    inputFiles :+ InputFile(
      file,
      config("annotation_gff_md5", freeVar = true, extraSubPath = geneAnnotationSubPath))
    file
  }
}
...
trait AnnotationRefFlat extends BiopetQScript with Reference { qscript: QScript =>
  ...
  lazy val annotationRefFlat = new LazyCheck({
    val file: File =
      config("annotation_refflat", freeVar = true, extraSubPath = geneAnnotationSubPath)
    inputFiles :+ InputFile(
      file,
      config("annotation_refflat_md5", freeVar = true, extraSubPath = geneAnnotationSubPath))
    file
  })
}
...
trait RibosomalRefFlat extends BiopetQScript with Reference { qscript: QScript =>
  ...
  lazy val ribosomalRefFlat = new LazyCheck({
    val file: Option[File] =
      config("ribosome_refflat", freeVar = true, extraSubPath = geneAnnotationSubPath)
    file match {
      case Some(f) =>
        inputFiles :+ InputFile(
          f,
          config("ribosome_refflat_md5", freeVar = true, extraSubPath = geneAnnotationSubPath))
      case _ =>
    }
    file
  })
