...
trait Reference extends Configurable {
  ...
  def referenceName: String = {
    parent match {
      ...
      case _ =>
        val default: String =
          config("default", default = "unknown_ref", path = List("references", referenceSpecies))
  ...
  lazy val dbsnpSubPath: List[String] =
    dbsnpVersion.map(List("dbsnp_annotations", _)).getOrElse(Nil)
  ...
  def referenceFasta(): File = {
    ...
    if (config.contains("reference_fasta")) checkFasta(file)
      ...
      def getReferences(map: Map[String, Any]): Set[(String, String)] =
        (for ((species, species_content) <- map
                .getOrElse("references", Map[String, Any]())
      ...
      if (!references.contains((referenceSpecies, referenceName))) {
        ...
        if (references.exists(_._1 == referenceSpecies)) {
          buffer.append(
            s"Reference: '$referenceName' does not exist in config for species: '$referenceSpecies'")
  ...
  def referenceSummary: Map[String, Any] = {
    ...
    Map(
      "contigs" ->
        (for (seq <- referenceDict.getSequences)
          yield
            seq.getSequenceName -> {
  ...
  def checkFasta(file: File): Unit = {
    if (!Reference.checked.contains(file)) {
      if (!file.exists())
        Logging.addError(
          s"Reference not found: $file, species: $referenceSpecies, name: $referenceName, configValue: " + config(
            "reference_fasta"))
...
object Reference {
  ...
  def askReference: Map[String, Any] = {
    val warn =
      "If you use a non-standard reference, please make sure that you have generated all required indexes for this reference"
    val globalSpecies =
      Config.global.defaults.getOrElse("references", Map()).asInstanceOf[Map[String, Any]]
    val species = Question.string(
      "species",
      description = Some(
        if (globalSpecies.nonEmpty)
    ...
    val referenceName = Question.string(
      "reference_name",
      description = Some(
        if (globalReferences.nonEmpty)
    ...
    val referenceFasta: Option[String] =
      if (reference.contains("reference_fasta")) None
      else {
        Some(
          Question.string(
            "Reference Fasta",
    ...
    Map("species" -> species, "reference_name" -> referenceName) ++ referenceFasta.map(
      "reference_fasta" -> _)
  }
}
