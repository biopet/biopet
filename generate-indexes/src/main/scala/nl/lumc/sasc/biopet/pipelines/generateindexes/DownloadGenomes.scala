...
class DownloadGenomes(val parent: Configurable) extends QScript with BiopetQScript {
  ...
  def init(): Unit = {
    if (referenceConfig == null)
      referenceConfig = referenceConfigFiles.foldLeft(Map[String, Any]())((a, b) =>
        ConfigUtils.mergeMaps(a, ConfigUtils.fileToConfigMap(b)))
  }
                ...
                genomeConfig.get("ncbi_assembly_report") match {
                  case Some(assemblyReport: String) =>
                    ...
                    downloadAssembly.mustHaveOne = genomeConfig
                      ...
                      .flatMap(x => x.map(y => y._1 + "=" + y._2))
                      .toList
                    downloadAssembly.mustNotHave = genomeConfig
                      .get("ncbi_assembly_must_not_have")
                      .map(_.asInstanceOf[util.ArrayList[util.LinkedHashMap[String, String]]])
                      .getOrElse(new util.ArrayList())
                      .flatMap(x => x.map(y => y._1 + "=" + y._2))
                      .toList
                  ...
                  case _ =>
                    val fastaUris = genomeConfig.getOrElse(
                      "fasta_uri",
                      throw new IllegalArgumentException(
                        s"No fasta_uri found for $speciesName - $genomeName")) match {
                ...
                if (downloadAnnotations) {
                          ...
                          x._2 match {
                            ...
                            case _ =>
                              throw new IllegalStateException(
                                s"values in the tag $tag should be json objects")
                        })
                      case None => Map()
                      case x =>
                        throw new IllegalStateException(
                  ...
                  getAnnotation("dbsnp").foreach {
                    case (version, dbsnp) =>
                      ...
                      val contigMap =
                        dbsnp.get("dbsnp_contig_map").map(_.asInstanceOf[Map[String, Any]])
                      ...
                      def addDownload(uri: String): Unit = {
                        ...
                        val output =
                          new File(dbpsnpDir,
                                   version + "." + new File(uri).getName + (if (isZipped) ""
                                                                            else ".gz"))
                        val curl = new Curl(this)
                        curl.url = uri
                        ...
                        val downloadCmd = (isZipped, contigSed) match {
                          case (true, Some(sed)) =>
                            curl | Zcat(this) | sed | new Bgzip(this) > output
                      ...
                      dbsnp.get("vcf_uri") match {
                        ...
                        case None =>
                          throw new IllegalStateException(
                            "Dbsnp should always have a 'vcf_uri' key")
                      }
                  ...
                  getAnnotation("gene_annotation").foreach {
                    case (version, geneAnnotation) =>
                      ...
                      val gffFile: Option[File] = geneAnnotation.get("gff_uri").map { gtfUri =>
                        val outputFile =
                          new File(dir, new File(gtfUri.toString).getName.stripSuffix(".gz"))
                        val curl = new Curl(this)
                      ...
                      val refFlatFile: Option[File] = gtfFile.map { gtf =>
                        ...
                        add(gtfToGenePred | Awk(
                          this,
                          """{ print $12"\t"$1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9"\t"$10 }""") > refFlat)
