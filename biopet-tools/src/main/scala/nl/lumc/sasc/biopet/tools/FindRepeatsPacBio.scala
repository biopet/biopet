...
object FindRepeatsPacBio extends ToolCommand {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (bedLine <- Source.fromFile(commandArgs.inputBed).getLines();
      ...
      val interval =
        new QueryInterval(bamHeader.getSequenceIndex(values(0)), values(1).toInt, values(2).toInt)
      ...
      for (result <- results) {
        if (result.isEmpty) notSpan += 1
          ...
          val length = oriRepeatLength - result.get.beginDel - result.get.endDel -
            (0 /: result.get.dels.map(_.length))(_ + _) + (0 /: result.get.ins
            .map(_.insert.length))(_ + _)
  ...
  def procesSamrecord(samRecord: SAMRecord, interval: QueryInterval): Option[Result] = {
    ...
    while (refPos < interval.end) {
      ...
      if (refPos > interval.end) {
        ...
        val insert = for (t <- oldReadPos + 1 until readPos)
          yield samRecord.getReadBases()(t - 1).toChar
