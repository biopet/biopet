...
class SageCreateTagCountsTest extends TestNGSuite with MockitoSugar with Matchers {
  ...
  def testMain(): Unit = {
    ...
    noException should be thrownBy main(
      Array(
        ...
        tagLib,
        "--countSense",
        sense.getAbsolutePath,
        "--countAllSense",
        allSense.getAbsolutePath,
        "--countAntiSense",
        antiSense.getAbsolutePath,
        "--countAllAntiSense",
        allAntiSense.getAbsolutePath
      ))
    noException should be thrownBy main(
      Array(
        "-I",
    ...
    noException should be thrownBy main(
      Array("-I",
            ...
            allSense.getAbsolutePath))
    noException should be thrownBy main(
      Array("-I", input, "--tagLib", tagLib, "--countSense", sense.getAbsolutePath))
  ...
  def testOutput(): Unit = {
    ...
    Source.fromFile(allAntiSense).mkString should equal(
      "ENSG00000254767\t75\nENSG00000255336\t65\n")
  }
