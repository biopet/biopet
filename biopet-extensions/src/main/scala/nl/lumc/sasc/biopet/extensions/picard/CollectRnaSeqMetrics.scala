...
class CollectRnaSeqMetrics(val parent: Configurable) extends Picard with Summarizable {
  ...
  @Argument(doc = "Minimum length of transcripts to use for coverage-based values calculation",
            required = false)
  ...
  @Argument(doc = "Minimum overlap percentage a fragment must have to be considered rRNA",
            required = false)
  ...
  override def beforeGraph(): Unit = {
    ...
    strandSpecificity match {
      case Some(s) =>
        require(
          validFlags.contains(s),
          s"Invalid Picard CollectRnaSeqMetrics strand specificity flag: $s. Valid values are " + validFlags
            .mkString(", "))
      case None => ;
    }
  }
