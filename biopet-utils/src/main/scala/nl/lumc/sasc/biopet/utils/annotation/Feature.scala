...
object Feature {
  def fromLine(line: String): Feature = {
    ...
    require(values.size == 9 || values.size == 8, s"A Gtf line should have 9 columns, gtf line: $line")

    val strand = values(6) match {
      case "+" => Some(true)
      case "-" => Some(false)
      case "." => None
      case _   => throw new IllegalArgumentException(s"strand only allows '+' or '-', not ${values(6)}, gtf line: $line")
    }
    ...
    val artibutes = values.lift(8).map(_.split(";")).getOrElse(Array()).map {
      ...
      case _ => throw new IllegalArgumentException(s"Atribute it not correct formatted, gtf line: $line")
    } toMap
    ...
    val frame = values(7) match {
      ...
      case s => throw new IllegalArgumentException(s"'$s' can not be parsed as frame, gtf line: $line")
    }

    Feature(values(0), values(1), values(2), values(3).toInt, values(4).toInt, score, strand, frame, artibutes)
  }
